...
| 数据源 | 示例 |
| --- | --- |
| 表格与日志 | CSV、TSV、TXT/LOG/DAT/LST、JSONL |
| 计算机视觉 | 图片文件或图片目录，可配检测标注 sidecar |
| 点云 | PLY、PCD、NPY、NPZ、XYZ、XYZN、XYZRGB、PTS、ASC 文件或帧目录 |
| 机器人 Recording | MCAP 与 ROS2 DB3 bag，包括分片 bag 目录 |
...
| 其他 Linux x64 | `DataScope-Studio-v0.3.0-linux-x86_64.AppImage` |
...
2. 选择表格/日志、图片、点云、MCAP 或 ROS2 DB3 数据源。
...
“打开项目包”重新载入。可在“设置”或转换任务中配置 Rerun 产物目录；配置后
`.rrd` 与 `.rbl` 会一起保存到指定目录，留空时继续使用项目内的
`recordings/` 与 `blueprints/`。

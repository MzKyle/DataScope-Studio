- **首页**
  - [项目简介](/)

- **快速入门**
  - [环境依赖](guide/prerequisites.md)
  - [开发运行](guide/run-app.md)
  - [安装与打包](guide/package-install.md)

- **系统架构**
  - [架构总览](architecture/README.md)
  - [模块全景](architecture/module-overview.md)
  - [数据流](architecture/data-flow.md)
  - [状态模型](architecture/state-model.md)

- **模块详解**
  - [总览](modules/README.md)
  - [数据适配器](modules/adapters.md)
  - [Workspace 与 Catalog](modules/workspace-catalog.md)
  - [Rerun 集成](modules/rerun-integration.md)
  - [查询系统](modules/query.md)
  - [插件与模板](modules/plugin-template.md)
  - [桌面端](modules/desktop.md)
  - [项目包](modules/project-packages.md)

- **代码导读**
  - [代码导读索引](code/README.md)
  - [Python 包](code/python-packages.md)
  - [FastAPI 服务](code/api-service.md)
  - [桌面前端](code/desktop-frontend.md)

- **接口参考**
  - [HTTP API](interfaces/api.md)
  - [CLI 命令](interfaces/cli.md)
  - [Mapping YAML](interfaces/mapping-yaml.md)

- **工作流程**
  - [导入与转换](workflow/import-convert.md)
  - [查询与导出](workflow/query-export.md)
  - [项目导出与导入](workflow/package-import-export.md)
  - [测试验收](workflow/testing.md)

- **日志与诊断**
  - [故障排查 FAQ](faq/troubleshooting.md)

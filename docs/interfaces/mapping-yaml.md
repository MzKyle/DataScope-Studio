...
`sort` 支持 `source` 和 `ascending`。`ascending` 会在转换和查询索引阶段按
标准化时间稳定排序，空值或无法解析的值置后；源文件不会被修改，`row`
sequence 按最终输出顺序重新编号。
...
每条校验 issue 都包含 `recommendation` 和 `suggestions`。建议操作是受限的
mapping 草稿修改，例如选择时间字段/单位/排序、替换 source field、修改
semantic type 或 entity path、启用/禁用 stream。校验 API 只返回建议，不会
直接修改已保存的 mapping。

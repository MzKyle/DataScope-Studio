...
| Method | Path | 说明 |
| --- | --- | --- |
| `GET` | `/api/status` | API 版本、`rerun_version` 与 `rerun_features`，用于隐藏或拒绝当前 runtime 不支持的 Rerun 0.33 选项 |
| `GET` | `/api/health` | 健康检查 |
...
默认不传 MCAP decoder、不运行 `rrd optimize`、只做 basic artifact 校验，以保持兼容。
